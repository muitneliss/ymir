...
Version source of truth is `plugins/ymir/.claude-plugin/plugin.json`'s `$.version` field, which release-please keeps in sync with the release tag. It is the only version number the runtime reads: the hook builds the asset URL from it and stamps it beside the binary, and `wiki --version` and self-reports read that stamp back. `wiki-cli/package.json` declares no version at all — the package is private and never published, and an independent number there drifted seven releases behind before it was removed.
